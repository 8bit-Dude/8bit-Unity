...
#if defined __LYNX__
  extern unsigned char sharedNum;    // Variable auto-generated by Build Script
  extern unsigned int sharedName[];  //					"
  extern unsigned int sharedData[];  //					"
#endif
...
void LoadNavigation(char *filename)
{
	...
	SedoricRead(filename, buffer);
#elif defined __LYNX__
	unsigned char *buffer;
	for (i=0; i<sharedNum; i++) {
		if (!strcmp(filename, sharedName[i])) {
			buffer = sharedData[i];	
		}
	}
#endif

#if (defined __ATMOS__) || (defined __LYNX__)
	// Read Lineup
	n = 1;
